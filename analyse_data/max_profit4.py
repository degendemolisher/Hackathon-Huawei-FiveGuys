...
def result_to_df(result_x, result_z, step_size, start_step, time_steps, solver):
    ...
    x_solution = np.array([solver.getVal(result_x[i]) for i in range(len(result_x))])
            ...
            if(i%24==0 and (i!=0 and i!=168)):
                ...
                for number in range(0,length,28):
                    for j in result_z[i][number+dc*7:number+dc*7+7]:
                        tot_z.append(int(solver.getVal(j)))
            else:
                tot_z.append(0)
...
def profit(demand, x, z, y, cumsum_x, step_size, TIMESTEPS, START_STEP):
    ...
    mult_array = np.arange(1,97)*1/96
...
def max_profit(demand, step_size=6, START_STEP=1, TIMESTEPS=168):
    ...
    # Variables
    # x is the bought servergens at each timestep chunk/checkpoint shape:timesteps,datacenter,servergen
    x = []
    ...
    for i in range(chunks):
        ...
        for k in range(4):
            #generate servers
            for j in range(7):
                x.append(solver.addVar(vtype="I", name=f"x{c}"))
                c+=1
    
    c=0
    for i in range(TIMESTEPS2):
        for latency in range(3):
            #generate demand met at each ts
            for j in range(7):
                y.append(solver.addVar(vtype="I", name=f"y{c}"))
                c+=1
            ...
            for window in range(x_number.shape[0]):
                for dc in range(x_number.shape[1]):
                    for s_gen in range(x_number.shape[2]):
                        temp_z = solver.addVar(vtype="I", name=f"z{c}")
                        tempz.append(temp_z)
                        c+=1
    ...
    for dc in range(4):
        ...
        for servergen in range(7):
            ...
            for j in action_time_array:
                if(j < rt[0] or j > rt[1]):
                    solver.addCons(x[start_pos+counter] == 0, name="lin_cons")
                counter+=28
    ...
    for timestep in range(TIMESTEPS2):
        ...
        if(timestep%step_size==0):
            for datacenter in range(4):
                ...
                solver.addCons(np.sum(cpu)+np.sum(gpu) <= dc_cap[datacenter], name="lin_cons")
                solver.addCons(np.sum(cpu)+np.sum(gpu) >= 0, name="lin_cons")
                ...
                if(datacenter != 3):
                    ...
                    for servergen in range(7):
                        ...
                        if(datacenter == 2):
                            ...
                            solver.addCons(y[index] <= x_minus_z_dc3*capacity[servergen]+x_minus_z_dc4*capacity[servergen], name="lin_cons")
                            solver.addCons(y[index] <= sens_demand[timestep][servergen], name="lin_cons")
                            solver.addCons(y[index] >= 0, name="lin_cons")
                        else:
                            #demand met constraint
                            x_minus_z = ls_cumsum[datacenter][timestep][servergen]-discards[servergen]
                            #dc(xa-za)*cap<=demand
                            solver.addCons(y[index] <= x_minus_z*capacity[servergen], name="lin_cons")
                            solver.addCons(y[index] <= sens_demand[timestep][servergen], name="lin_cons")
                            solver.addCons(y[index] >= 0, name="lin_cons")
            else:
                if(datacenter != 3):
                    for servergen in range(7):
                        ...
                        if(datacenter == 2):
                            ...
                            solver.addCons(y[index] <= x_dc3*capacity[servergen]+x_dc4*capacity[servergen], name="lin_cons")
                            solver.addCons(y[index] <= sens_demand[timestep][servergen], name="lin_cons")
                            solver.addCons(y[index] >= 0, name="lin_cons")
                        else:
                            #demand met constraint
                            solver.addCons(y[index] <= ls_cumsum[datacenter][timestep][servergen]*capacity[servergen], name="lin_cons")
                            solver.addCons(y[index] <= sens_demand[timestep][servergen], name="lin_cons")
                            solver.addCons(y[index] >= 0, name="lin_cons")
                for servergen in range(7):
                    x_minus_z_1to1 = temp_x[chunk][datacenter][servergen] - re_z1_cumsum[5][chunk][datacenter][servergen]
                    # print(x_minus_z_1to1)
                    solver.addCons(x_minus_z_1to1 >= 0, name="lin_cons")
                    # solver.addCons(x_minus_z_1to1 <= dc_cap[datacenter], name="lin_cons")
            curr_max = min(max_in_existance, chunk+1)
            for servergen in range(7):
                for server in range(curr_max):
                    s = x_minus_z_arr[timestep][datacenter][servergen][server]
                    solver.addCons(s >= 0, name="lin_cons")
                    # solver.addCons(s <= dc_cap[datacenter], name="lin_cons")
    
    revenues, costs = profit(demand2, x, z, y, ls_cumsum, step_size, TIMESTEPS2, START_STEP)
    ts_profits = []
    for timestep in range(TIMESTEPS2):
        ts_profit = revenues[timestep]-costs[timestep]
        ts_profits.append(ts_profit)

    # Objective
    solver.setObjective(quicksum(ts_profits[i] for i in range(TIMESTEPS2)), "maximize")
    # solver.setObjective(objective_func(demand2, x, z, y, ls_cumsum, step_size, TIMESTEPS2, START_STEP), "maximize")
    
    # Solve
    solver.optimize()
    # status = solver.Solve()
    if solver.getStatus() == "optimal":
        print('Total value =', make_readable(solver.getObjVal()))
    else:
        print('The problem does not have an optimal solution.')

    if solver.getStatus() == "optimal":
        result_df = result_to_df(x,z,step_size,START_STEP2,TIMESTEPS2, solver)
    else:
        return "no solution"
