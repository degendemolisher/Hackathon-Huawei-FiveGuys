...
def greedy_profit_algorithm(actual_demand: pd.DataFrame, quantile: float, merge_threshold_multiplier: float = 4):
    ...
    for server_generation, latency_sensitivity in sorted_servers:
        ...
        while True:
            ...
            for i in reversed(sorted_ranges_i):
                ...
                results["fleet"].extend(validated_results)

                # 3.1) If there are leftover servers to buy and the latency is high, perform the Remaining Slot Decrement Algorithm with DC4
                if latency_sensitivity == 'high' and leftover_buy_count > 0:
                    ...
                    results["fleet"].extend(dc4_results)
            ...
            if len(ranges) == 0:
                results_df = pd.DataFrame(results["fleet"])
    ...
    # remaining_slots.to_csv('./output/remaining_slots.csv', sep='\t', encoding='utf-8')
