...
def srvs_count(fleet_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # srvs_count = srvs_count[LATENCIES].reset_index()
    
    # # Sort the DataFrame by server generation
    # srvs_count = srvs_count.sort_values('server_generation')
...
def check_fleet_for_old_servers(state: SystemState) -> List[Dict]:
    ...
    Returns:
        List[Dict]: A list of dismissal actions. Each action is a dictionary with the following keys:
...
def move_servers(state: SystemState, calculate_demand_def_exc: pd.DataFrame, actions: List) -> List:
    """
    ...
    Args:
        ...
        calculate_demand_def_exc (pd.DataFrame): DataFrame with demand deficit and excess calculations.
        
    Returns:
        list: List of dictionaries containing server movement actions.
        pd.DataFrame: updated calculate_demand_def_exc to find which demand yet to satisfy
    """
    
    fleet_df = state.fleet
    datacenters_df = state.datacenter_capacity.copy()
    updated_demand_def_exc = calculate_demand_def_exc.copy()

    for idx, row in calculate_demand_def_exc.iterrows():
    ...
    return actions, updated_demand_def_exc


def buy_servers(state: SystemState, demand_def_exc: pd.DataFrame, actions: List) -> List:
    """
...
def allocate_servers(state: SystemState, demand: pd.DataFrame) -> List[Dict]:
    ...
    actions, updated_demand_def_exc = move_servers(state, demand_def_exc, actions)

    state.update_fleet(actions)
    state.update_datacenter_capacity()

    actions = buy_servers(state, updated_demand_def_exc, actions)
...
def main():
    ...
    demand, datacenters, servers, _ = load_problem_data()
